 ...
 * @@ Objective (board.c)
 *
 * This file contains all of the function and support code for the multi-node
 * board system originally coded by Erwin Andreasen.
 *
 */
...
#define MAX_LINE_LENGTH 80	/* enforce a max length of 80 on text lines, 
							 * reject longer lines. This only applies in the 
							 * body of the note */
...
BOARD_DATA boards[MAX_BOARD] = {
	...
	{"Announce", "Announcements from Immortals", 0, L8, "all",
	DEF_NORMAL, 60, NULL, FALSE}
	,
		
	{"Bugs", "Typos, bugs, errors", 0, 2, "imm",
	DEF_NORMAL, 60, NULL, FALSE}
	,
		
	{"Personal", "Personal messages", 0, 2, "all",
	DEF_EXCLUDE, 28, NULL, FALSE}
	,
		
	{"History", "Character Histories", 0, 2, "all",
	DEF_INCLUDE, -1, NULL, FALSE}
	,
		
	{"Q/A", "Question/Answer forum", 0, 2, "all",
	DEF_INCLUDE, 7, NULL, FALSE}
	,
		
	{"Builders", "Building Discussions", L8, L8, "imm",
	DEF_NORMAL, 28, NULL, FALSE}
	,
		
	{"Immortal", "Immortal Discussions", L8, L8, "imm",
	DEF_NORMAL, 28, NULL, FALSE}
	,
		
	{"Policy", "Policy Files", 0, L8, "all",
	DEF_INCLUDE, -1, NULL, FALSE}
};

/* The prompt that the player is sent after finishing a note with ~ or END */
const char *szFinishPrompt =
"(`WC`w)ontinue, (`WV`w)iew, (`WP`w)ost or (`WF`w)orget it?";
...
void announce_note (CHAR_DATA * sender, BOARD_DATA * board, NOTE_DATA * note)
{
	...
	for (d = descriptor_list; d != NULL; d = dnext)
	{
		...
		if (vch->position > POS_DEAD && is_note_to (vch, note) && vch != sender)
		{
			chprintf (vch, "A note has been posted by %s "
				"on the '%s' board for you.\n\r",
				PERS (sender, vch), board->short_name);
		}
...
void finish_note (BOARD_DATA * board, NOTE_DATA * note)
{
	...
	if (!fp)
	{
		bugf ("[board.c::finish_note] could not open one of the note files in append mode");
...
static void unlink_note (BOARD_DATA * board, NOTE_DATA * note)
{
	...
	else
	{
		...
		if (!p)
			bugf("[board.c::unlink_note] could not find note.");
		else
			p->next = note->next;
	}
}
...
static void save_board (BOARD_DATA * board)
{
	...
	if (!fp)
	{
		bugf("[board.c::save_board] Error writing to: %s", filename);
	}
	else
	{
...
static void load_board (BOARD_DATA * board)
{
	...
	for (;;)
	{
		...
		if (board->purge_days > -1 && pnote->expire < current_time)
		{
			...
			if (!fp_archive)
				bugf("[board.c::load_board] Could not open archive boards for writing");
			else
			{
				append_note (fp_archive, pnote);
				fclose (fp_archive);	/* it might be more efficient to 
				* close this later */
			}
	...
	bugf ("[board.c::load_notes] bad key word.");
	return;			/* just return */
}
...
bool is_note_to (CHAR_DATA * ch, NOTE_DATA * note)
{
	...
	if ((get_trust (ch) >= L2) && (is_exact_name ("council", note->to_list)))
		return TRUE;
...
_DOFUN (do_nwrite)
{
	...
	{
		send_to_char ("Note in progress cancelled because you did not manage to"
	...
	act ("`G$n starts writing a note.`w", ch, NULL, NULL, TO_ROOM, POS_RESTING);
	
	/* Begin writing the note ! */
	if (!IS_FLAG (ch->states.config, _AFK))
	{
		send_to_char ("Placing you in AFK mode.\n\r\n\r", ch);
		...
		/* Are we continuing an old note or not? */
	{
		...
		case DEF_NORMAL:
			sprintf (buf,
				"If you press Return, default recipient \"`W%s`w\" will be chosen.\n\r",
				ch->pcdata->board->names);
			break;
		case DEF_INCLUDE:
			sprintf (buf,
				"The recipient list MUST include \"`W%s`w\". If not, it will be added automatically.\n\r",
				ch->pcdata->board->names);
			break;
			
		case DEF_EXCLUDE:
			sprintf (buf,
				"The recipient of this note must NOT include: \"`C%s`w\".",
				ch->pcdata->board->names);
		...
		/* we are continuing, print out all the fields and the note so far */
	{
		sprintf (buf, "`Y\n\rTo`w: %s\n\r"
			...
			(ch->pcdata->board->purge_days == -1) ? "Never" :
		...
		note_append (ch, &ch->pcdata->in_progress->text);
	}
...
_DOFUN (do_nread)
{
		...
		/* just next one */
	{
		...
		for (p = ch->pcdata->board->note_first; p; p = p->next, count++)
			...
			{
				sprintf (buf, "Reset to first board.\n\r");
...
_DOFUN (do_nlist)
{
	...
	for (p = ch->pcdata->board->note_first; p; p = p->next)
	{
		...
		if (is_note_to (ch, p))
		{
			...
			{
				sprintf (buf, "`W%3d`w] `B%c`Y%-13s`y %s`w \n\r",
					num,
...
_DOFUN (do_nclear)
{
	if (ch->pcdata->in_progress)
	{
		...
		send_to_char ("You dont have any notes in progres.\n\r", ch);
}
...
_DOFUN (do_note)
{
	...
	else if (!str_cmp (arg, "write") || !str_cmp (arg, "to"))
...
_DOFUN (do_board)
{
	...
	if (!argument[0])		/* show boards */
	{
		...
		count = 1;
		send_to_char ("\n\r`RNum         Name Unread Description`w\n\r"
			"`r=== ============ ====== ===========`w\n\r", ch);
		for (i = 0; i < MAX_BOARD; i++)
		{
				...
				const char *subject, const int expire_days, const char *text)
{
	...
	{
		bugf("[board.c::make_note] board not found");
		return;
	}
	
	if (strlen (text) > MAX_NOTE_TEXT)
	{
		bugf("[board.c::make_note] text too long (%d bytes)", strlen (text));
		return;
	}
	...
	if (expire_days < 0)
		note->expire = current_time + (365 * 5) * 60 * 60 * 24;
	else
...
void handle_con_note_to (DESCRIPTOR_DATA * d, char *argument)
{
	...
	if (!ch->pcdata->in_progress)
	{
		...
		bugf("[board.c::handle_con_note_to] no note in progress", 0);
		return;
	}
	...
	case DEF_INCLUDE:		/* forced default */
		if (!is_exact_name (ch->pcdata->board->names, buf))
		{
			...
			sprintf (buf, "\n\rYou did not specify %s as recipient, so it was"
				" automatically added.\n\r`YNew To`w :  %s\n\r",
	...
	case DEF_EXCLUDE:		/* forced exclude */
		if (is_exact_name (ch->pcdata->board->names, buf))
		{
			sprintf (buf, "You are not allowed to send notes to %s on this board."
				" Try again.\n\r`YTo`w:      ", ch->pcdata->board->names);
			write_to_buffer (d, buf, 0);
...
void handle_con_note_subject (DESCRIPTOR_DATA * d, char *argument)
{
	...
	{
		d->connected = CON_PLAYING;
		bugf ("[board.c::handle_con_note_subject] no note in progress", 0);
		return;
	}
		...
		/* advance to next stage */
	{
		...
		{
			sprintf (buf, "\n\rHow many days do you want this note to expire"
				" in?\n\rPress Enter for default value for this "
				"board, `W%d`w days.\n\r`YExpire`w:  ",
		...
		{
			(ch->pcdata->board->purge_days == -1) ?
				(ch->pcdata->in_progress->expire =
				current_time + (365 * 5) * 24L * 3600L) :
			(ch->pcdata->in_progress->expire =
			...
			note_edit (ch, &ch->pcdata->in_progress->text);
		}
	}
}
...
void handle_con_note_expire (DESCRIPTOR_DATA * d, char *argument)
{
	...
	{
		d->connected = CON_PLAYING;
		bugf ("[board.c::handle_con_note_expire] no note in progress", 0);
		return;
	}
	...
	if (!buf[0])			/* assume default expire */
		...
		/* use this expire */ if (!is_number (buf))
	{
		write_to_buffer (d, "Write the number of days!\n\r", 0);
		write_to_buffer (d, "`YExpire`w:  ", 0);
		return;
	}
...
/** Function: note_edit
  * Descr   : Display help header to player entering note editor and
  *         : place them into editor.
  * Written : v1.0 4/98
...
void note_edit (CHAR_DATA * ch, char **pString)
{
	...
	send_to_char(
		"`g--------------------------------------------------------------`w\n\r"
		"`g--`w Type .h on a new line for help with this editor, or enter`g--`w\n\r"
		"`g--`w~ or @ on a new line when finished writing your note.     `g--`w\n\r"
		"`g--------------------------------------------------------------`w\n\r",ch);
...
/** Function: note_append
  * Descr   : Places player into append mode, displays help header and the
  *         : current contents of the note in progress.
  * Written : v1.0 4/98
  * Author  : Gary McNickle <gary@tarmongaidon.org>
  */
...
{
	send_to_char(
		"`g--`w Editing note in progress `g------------------------------`w\n\r"
		"`g--`w Type .h on a new line for help with this editor, or enter`g--`w\n\r"
		"`g--`w~ on a new line when finished writing your note.          `g--`w\n\r"
		"`g--------------------------------------------------------------`w\n\r",ch);
...
void handle_con_note_text (DESCRIPTOR_DATA * d, char *argument)
{
	...
	if (ch->desc->pString == NULL)	/* recently finished note, post it now */
	{
		if (ch->pcdata->in_progress->text != NULL &&
			strlen (ch->pcdata->in_progress->text) > 1)
		{
			/* note was not cleared/cancelled */
			...
			if (IS_FLAG (ch->states.config, _AFK))
			{
				send_to_char ("Removing AFK status.\n\r", ch);
				REMOVE_FLAG (ch->states.config, _AFK);
			}
			
			announce_note (ch, ch->pcdata->board, ch->pcdata->in_progress);
			ch->pcdata->in_progress = NULL;
			act ("`G$n finishes $s note.`w", ch, NULL, NULL, TO_ROOM, POS_RESTING);
		}
		else
		{
