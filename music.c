 ...
 * @@ Objective (music.c)
 * 
 * This file contains the support code for musical instruments.
 */
...
void song_update (void)
{
  ...
  if (channel_songs[1] > -1)
  {
	...
	if (d->connected == CON_PLAYING
	  ...
	  act ("$t", d->character, buf, NULL, TO_CHAR, POS_RESTING);
      }
    }
  }
...
/** Function: load_songs
  * Descr   : Reads in the song database.
  * Note    : bug with fopen/fclose fixed.
  * Returns : void
  * Syntax  : (n/a)
  * Written : v1.0 8/98
...
/*
    ...
    sprintf(file, "%s%s", DATA_DIR, MUSIC_FILE);
    if ((fp = fopen(file,"r")) == NULL)
    {
	bugf("Couldn't open music file, no songs available.",0);
	return;
    }
	...
	for ( ; ;)
	{
	    ...
	    song_table[count].lyrics[lines] = fread_string(fp);
	    lines++;
	}
    }
  */
...
_DOFUN (do_play)
{
  ...
  {
    send_to_char ("The musicbox is full up right now.\n\r", ch);
    return;
  }
