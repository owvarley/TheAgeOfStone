...
void load_social_table ()
{
  ...
  sprintf (file, "%s%s", DATA_DIR, SOCIAL_FILE);
  fp = fopen (file, "r");

  if (!fp)
  {
    bugf ("[sedit.c::load_social_table] Could not open %s for reading.", file);
    _quit (1);
  }
...
bool save_social_table ()
{
  ...
  if (!fp)
  {
    bugf ("[sedit.c::save_social_table] Could not open for writing.", file);
    return FALSE;
  }
