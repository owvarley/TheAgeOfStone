...
/**
 * @@ Objective (mob_prog.c)
 * 
 * Contains the mob_program triggers and support functions
 */
...
int num_eval (int lval, int oper, int rval)
{
  switch (oper)
  {
	  ...
	  case EVAL_LT:
		return (lval < rval);
	  default:
		bugf("[mob_prog.c::num_eval] invalid operator (%d)", oper);
		return 0;
  }
}
 ...
 *----------------------------------------------------------------------
 */
long cmd_eval (long vnum, char *line, int check,
	      ...
	      const void *arg1, const void *arg2, CHAR_DATA * rch)
{
  ...
  if (rval >= 0)
  {
    ...
    {
      bugf("[mob_prog.c::cmd_eval] prog %ld syntax error(2) '%s'", vnum, original);
      return FALSE;
    }
  ...
  if (buf[0] != '$' || buf[1] == '\0')
  {
    bugf("[mob_prog.c::cmd_eval] prog %ld syntax error(3) '%s'", vnum, original);
    return FALSE;
  }
  else
    code = buf[1];
  ...
  switch (code)
  {
	  ...
	  case 'q':
		...
		break;
	  default:
		bugf("[mob_prog.c::cmd_eval] prog %ld syntax error(4) '%s'", vnum, original);
		return FALSE;
  }
  /*
  ...
  case CHK_ISCHARM:		/* A relic from MERC 2.2 MOBprograms */
    return (lval_char != NULL && IS_AFFECTED (lval_char, _AFF_COMPULSION));
  case CHK_ISFOLLOW:
  ...
  case CHK_AFFECTED:
    return (lval_char != NULL
	    && IS_FLAG (lval_char->states.affected_by,
			flag_lookup (buf, _affected_by_bits)));
  case CHK_ACT:
    return (lval_char != NULL
	    && IS_FLAG (lval_char->states.player,
			flag_lookup (buf, _act_bits)));
  case CHK_IMM:
    return (lval_char != NULL
	    && IS_FLAG (lval_char->states.imm, 
	                flag_lookup (buf, _resistance_bits)));
  case CHK_OFF:
    return (lval_char != NULL
	    && IS_FLAG (lval_char->states.off, 
	                flag_lookup (buf, _off_bits)));
  case CHK_CARRIES:
    if (is_number (buf))
      return (lval_char != NULL
  ...
  case CHK_ISNATION:
      return(lval_char != NULL && ((LOWER(lval_char->pcdata->nation[0]) == LOWER(buf[0])) && !str_cmp(lval_char->pcdata->nation, buf)));
  case CHK_WEARS:
    if (is_number (buf))
      return (lval_char != NULL
  ...
  case CHK_CLASS:
    return (lval_char != NULL && lval_char->c_class == class_lookup (buf));
  case CHK_RACE:
  ...
  if ((oper = keyword_lookup (fn_evals, buf)) < 0)
  {
    bugf("[mob_prog.c::cmd_eval] prog %ld syntax error(5): '%s'", vnum, original);
    return FALSE;
  }
  one_argument (line, buf);
  rval = atoi (buf);
		 ...
		 const void *arg1, const void *arg2, CHAR_DATA * rch)
{
  ...
  while (*str != '\0')
  {
    ...
    {
    default:
      bugf("[mob_prog.c::expand_arg] bad code %c.", *str);
      i = " <@@@> ";
      break;
    case 'i':
	...
	? (IS_NPC (mob->mprog_target) ? mob->mprog_target->short_descr : mob->
	   mprog_target->name) : someone;
      break;
    case 'j':
    ...
    case 'e':
      i = (ch != NULL && can_see (mob, ch))
	? he_she[URANGE (0, ch->sex, 2)] : someone; break;
    case 'E':
      i = (vch != NULL && can_see (mob, vch))
		   ...
		   const void *arg2)
{
  ...
  {
    bugf("[mob_prog.c::program_flow] MAX_CALL_LEVEL exceeded, vnum %ld", mob->pIndexData->vnum);
    call_level--;
    return;
  }
  ...
  while (*code)
  {
    ...
    *b = *c = *d = '\0';
    ...
    if (!str_cmp (control, "if"))
    {
      ...
      {
		bugf ("[mob_prog.c::program_flow] misplaced if statement, mob %ld prog %ld", mvnum, pvnum);
		call_level--;
		return;
      }
      ...
      {
		bugf("[mob_prog.c::program_flow] Max nested level exceeded, mob %ld prog %ld", mvnum, pvnum);
		call_level--;
		return;
      }
	...
	cond[level] = cmd_eval (pvnum, line, check, mob, ch, arg1, arg2, rch);
      ...
      {
		bugf ("[mob_prog.c::program_flow] invalid if_check (if), mob %ld prog %ld", mvnum, pvnum);
		call_level--;
	return;
      }
    ...
    else if (!str_cmp (control, "or"))
    {
      ...
      {
		bugf ("[mob_prog.c::program_flow] or without if, mob %ld prog %ld", mvnum, pvnum);
		call_level--;
		return;
      }
      if (level && cond[level - 1] == FALSE)
	...
	eval = cmd_eval (pvnum, line, check, mob, ch, arg1, arg2, rch);
      ...
      {
		bugf("[mob_prog.c::program_flow] invalid if_check (or), mob %ld prog %ld", mvnum, pvnum);
		call_level--;
		return;
      }
    ...
    else if (!str_cmp (control, "and"))
    {
      if (!level || state[level - 1] != BEGIN_BLOCK)
      {
		bugf("[mob_prog.c::program_flow] and without if, mob %ld prog %ld", mvnum, pvnum);
		call_level--;
		return;
      }
      if (level && cond[level - 1] == FALSE)
	...
	eval = cmd_eval (pvnum, line, check, mob, ch, arg1, arg2, rch);
      ...
      {
		bugf ("[mob_prog.c::program_flow] invalid if_check (and), mob %ld prog %ld", mvnum, pvnum);
		call_level--;
		return;
      }
    ...
    else if (!str_cmp (control, "endif"))
    {
      if (!level || state[level - 1] != BEGIN_BLOCK)
      {
		bugf ("[mob_prog.c::program_flow] endif without if, mob %ld prog %ld", mvnum, pvnum);
		call_level--;
		return;
      }
      cond[level] = TRUE;
    ...
    else if (!str_cmp (control, "else"))
    {
      if (!level || state[level - 1] != BEGIN_BLOCK)
      {
		bugf("[mob_prog.c::program_flow] else without if, mob %ld prog %ld", mvnum, pvnum);
		call_level--;
		return;
      }
      if (level && cond[level - 1] == FALSE)
...
bool mp_exit_trigger (CHAR_DATA * ch, int dir)
{
  ...
  for (mob = ch->in_room->people; mob != NULL; mob = mob->next_in_room)
  {
	    ...
	    && can_see (mob, ch))
	{
	  ...
	  if (prg->trig_type == TRIG_EXALL && dir == atoi (prg->trig_phrase))
	{
	  program_flow (prg->vnum, prg->code, mob, ch, NULL, NULL);
	  return TRUE;
	}
